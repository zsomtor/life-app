...
export function TodayHero({ today }: { today: string }) {
  ...
  const complete = async (id: string) => {
    try {
      ...
      /* surfaced by the tasks card on next refresh */
    }
  };
  ...
  const teamToday = data ? [...data.teamTasks.overdue, ...data.teamTasks.dueToday].filter(
    (t) => t.assigned_to === "Zsomtor" || t.co_assignees?.includes("Zsomtor")
  ) : [];

  return (
    <section className="rise rounded-xl border border-line bg-gradient-to-b from-panel2 to-panel p-4">
      <div className="mb-3 flex flex-wrap items-baseline justify-between gap-2">
        ...
        {data && (
          <div className="flex items-center gap-3 font-mono text-[12px] text-mut">
            <span>{data.personalTasks.openCount} open tasks</span>
            <span>{data.calendar.length} events</span>
            <span className={data.ideaInboxCount > 0 ? "text-warn" : ""}>
              {data.ideaInboxCount} in inbox
            </span>
          </div>
        )}
      </div>
      ...
      {loading && !data ? (
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {/* Schedule */}
          <div>
            <p className="mb-1.5 text-[11px] font-semibold tracking-wider text-dim uppercase">Schedule</p>
              ...
              <ul className="space-y-1">
                ...
                {teamToday.map((t) => (
                  <li key={t.id} className="flex items-center gap-2.5 text-[13.5px] text-mut">
                    <span className="grid size-[18px] shrink-0 place-items-center text-[10px] text-acc">⇡</span>
      ...
      {data && data.errors.length > 0 && (
        <p className="mt-3 text-[11.5px] text-dim" title={data.errors.map((e) => `${e.source}: ${e.message}`).join("\n")}>
